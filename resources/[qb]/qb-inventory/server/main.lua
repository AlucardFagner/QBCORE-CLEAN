	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					...
					local message = string.format("**[ADMIN]** %s deu %d %s para %s", GetPlayerName(source), amount, itemData["name"], GetPlayerName(id), os.date("%H:%M:%S"))
					TriggerEvent("discordLogs", "Giveitem", message, 65280)
...
print("^2QB^7-^2INVENTORY ^2v^41^7.^41 ^7- ^2Inventory Rework by ^1MARFY^7")
-- This Package Leaked By TC HUB.
-- For More Leaks Join Our Discord Server: https://discord.gg/4KFYgmrYaG / https://discord.gg/4KFYgmrYaG
-- LEAKED BY TC HUB
