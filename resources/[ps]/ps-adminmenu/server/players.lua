...
RegisterNetEvent('ps-adminmenu:server:SetJob', function(data, selectedData)
    ...
    local src = source
    local playerId, Job, Grade = selectedData["Jogador"].value, selectedData["Emprego"].value, selectedData["Cargo"].value
    local Player = QBCore.Functions.GetPlayer(playerId)
    local name = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
    local jobInfo = QBCore.Shared.Jobs[Job]
    local grade = jobInfo["grades"][selectedData["Cargo"].value]

    if not jobInfo then
        TriggerClientEvent('QBCore:Notify', source, "Não é um trabalho válido", 'error')
        return
    end

    if not grade then
        TriggerClientEvent('QBCore:Notify', source, "Não é uma nota válida", 'error')
        return
    end
...
RegisterNetEvent('ps-adminmenu:server:SetGang', function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local playerId, Gang, Grade = selectedData["Jogador"].value, selectedData["Gangue"].value, selectedData["Cargo"].value
    local Player = QBCore.Functions.GetPlayer(playerId)
    local name = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
    local GangInfo = QBCore.Shared.Gangs[Gang]
    local grade = GangInfo["grades"][selectedData["Cargo"].value]

    if not GangInfo then
        TriggerClientEvent('QBCore:Notify', source, "Não é uma gangue válida", 'error')
        return
    end

    if not grade then
        TriggerClientEvent('QBCore:Notify', source, "Não é um cargo válido", 'error')
        return
    end
...
RegisterNetEvent("ps-adminmenu:server:SetPerms", function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local rank = selectedData["Permissões"].value
    local targetId = selectedData["Jogador"].value
...
RegisterNetEvent("ps-adminmenu:server:RemoveStress", function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local targetId = selectedData['Jogador (Opcional)'] and tonumber(selectedData['Jogador (Opcional)'].value) or src
    local tPlayer = QBCore.Functions.GetPlayer(tonumber(targetId))
